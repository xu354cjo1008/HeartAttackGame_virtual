...
struct servo *servo_create(enum servo_type type, int fadj, int max_ppb, int sw_ts)
{
    ...
    if (type == CLOCK_SERVO_PLL) {
		return pll_servo_create(fadj, max_ppb, sw_ts);
	}
    if (type == CLOCK_SERVO_FLL) {
		return fll_servo_create(fadj, max_ppb, sw_ts);
	}
    if (type == CLOCK_SERVO_ADJPLL) {
        return adjPll_servo_create(fadj, max_ppb, sw_ts);
    }
	return NULL;
}
