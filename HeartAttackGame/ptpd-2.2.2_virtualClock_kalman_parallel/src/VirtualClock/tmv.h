...
{
	return (tmv_t)ts.tv_sec * NS_PER_SEC + ts.tv_nsec;
}
static inline struct timespec tmv_to_timespec(tmv_t ts)
{
    struct timespec x;
    x.tv_sec = ts/NS_PER_SEC;
    x.tv_nsec = ts%NS_PER_SEC;
	return x;
}

static inline tmv_t timeinternal_to_tmv(TimeInternal ts)
{
	return (tmv_t)ts.seconds * NS_PER_SEC + ts.nanoseconds;
}

static inline TimeInternal tmv_to_timeinternal(tmv_t ts)
{
    TimeInternal x;
    x.seconds = ts/NS_PER_SEC;
    x.nanoseconds = ts%NS_PER_SEC;
	return x;
}

static inline tmv_t timeval_to_tmv(struct timeval tv)
{
...
{
    a->tv_sec += __iter_div_u64_rem(a->tv_nsec + ns, NS_PER_SEC, &ns);
    a->tv_nsec = ns;
}
